...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    if (req.file.size > 10485760) {
         return res.json({ success: false, message: "File size too large. Maximum is 10MB." });
      }
...
export const resumeReview = async (req, res) => {
  try {
    ...
    if(resume.size > 5 * 1024 * 1024){
      return res.json({ success: false, message: "File size should be less then 5MB" })
    }
    ...
    weaknesses, and areas for improvement.Resume Content: ${pdfData.text}`;
