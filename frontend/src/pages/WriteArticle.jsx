  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(data.success){
        ...
        toast.success("Write article successfully")
      }else{
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-xl border border-gray-200">
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {articleLength.map((item, index) => (
            ...
            ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700 border-blue-500' : 'bg-gray-50 text-gray-400 border-gray-400'}`} key={index}>{item?.text}</span>
          ))}
        </div>

        <button disabled={loading} className='w-full flex justify-center items-center gap-2 rounded-xl bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 mt-8
